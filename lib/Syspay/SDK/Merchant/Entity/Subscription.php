...
class Syspay_Merchant_Entity_Subscription extends Syspay_Merchant_Entity
{
    ...
    // Merchant stopped the subscription via his interface or via the API
    const END_REASON_UNSUBSCRIBED_MERCHANT = 'UNSUBSCRIBED_MERCHANT';
    // Admin stopped the subscription via the admin interface
    const END_REASON_UNSUBSCRIBED_ADMIN    = 'UNSUBSCRIBED_ADMIN';
    // Max retry attempts reached for one payment
    const END_REASON_SUSPENDED_ATTEMPTS    = 'SUSPENDED_ATTEMPTS';
    // Payment method has expired
    const END_REASON_SUSPENDED_EXPIRED     = 'SUSPENDED_EXPIRED';
    // A chargeback was received on this subscription
    const END_REASON_SUSPENDED_CHARGEBACK  = 'SUSPENDED_CHARGEBACK';
    // The subscription is complete
    const END_REASON_COMPLETE              = 'COMPLETE';
    ...
    public static function buildFromResponse(stdClass $response)
    {
        ...
        $subscription->setStartDate(
            isset($response->start_date)?Syspay_Merchant_Utils::tsToDateTime($response->start_date):null
        );
        $subscription->setEndDate(
            isset($response->end_date)?Syspay_Merchant_Utils::tsToDateTime($response->end_date):null
        );
        $subscription->setEndReason(
            isset($response->end_reason)?$response->end_reason:null
        );
