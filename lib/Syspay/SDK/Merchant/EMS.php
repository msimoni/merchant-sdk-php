...
class Syspay_Merchant_EMS
{
    ...
    public function getEvent()
    {
        ...
        if (!isset($content->data)) {
            throw new Syspay_Merchant_EMSException(
                'Unable to get data from content',
                Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
            );
        }
        ...
        switch ($content->type) {
            case 'payment':
                if (!isset($content->data->payment)) {
                    throw new Syspay_Merchant_EMSException(
                        'Payment event received with no payment data',
                        Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
                    );
                }
            ...
            case 'refund':
                if (!isset($content->data->refund)) {
                    throw new Syspay_Merchant_EMSException(
                        'Refund event received with no refund data',
                        Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
                    );
                }
            ...
            case 'chargeback':
                if (!isset($content->data->chargeback)) {
                    throw new Syspay_Merchant_EMSException(
                        'Chargeback event received with no chargeback data',
                        Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
                    );
                }
            ...
            case 'billing_agreement':
                if (!isset($content->data->billing_agreement)) {
                    throw new Syspay_Merchant_EMSException(
                        'Billing agreement event received with no billing_agreement data',
                        Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
                    );
                }
            ...
            case 'subscription':
                if (!isset($content->data->subscription)) {
                    throw new Syspay_Merchant_EMSException(
                        'Subscription event received with no subscription data',
                        Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
                    );
                }
    ...
    private function checkChecksum()
    {
        ...
        if (!isset($this->secrets[$this->headers['x-merchant']])) {
            throw new Syspay_Merchant_EMSException(
                'Unknown merchant: ' . $this->headers['x-merchant'],
                Syspay_Merchant_EMSException::CODE_UNKNOWN_MERCHANT
            );
        }

        if (!Syspay_Merchant_Utils::checkChecksum(
            $this->content,
            $this->secrets[$this->headers['x-merchant']],
            $this->headers['x-checksum']
        )) {
            throw new Syspay_Merchant_EMSException(
                'Invalid checksum',
                Syspay_Merchant_EMSException::CODE_INVALID_CHECKSUM
            );
        }
    }
    ...
    private function getContent()
    {
        switch ($this->headers['content-type']) {
            case 'application/json':
                ...
                if (false === $content) {
                    throw new Syspay_Merchant_EMSException(
                        'Unable to parse request body, invalid json',
                        Syspay_Merchant_EMSException::CODE_INVALID_CONTENT
                    );
                }
                return $content;
